...
function createScene() {
  ...
  /* Je voulais créer un tableau de score aui enregistre tous les dudes tués par le perso principal
  let scoreTexture = new BABYLON.DynamicTexture("scoreTexture", 512, scene, true);
  let scoreboard = BABYLON.Mesh.CreatePlane("scoreboard", 5, scene);
...
/* Code pour remplacer le tank par un Dude mais ne fonctionnant pas.. (main.js:257 Uncaught TypeError: Cannot read property 'name' of undefined)

function createTank(scene) {
  BABYLON.SceneLoader.ImportMesh("","models/Dude/","Dude.babylon",scene, (newMeshes, particleSystems, skeletons) => {
    let tank = newMeshes[0];
    tank.position = new BABYLON.Vector3(0, 0, 5);
    ...
    tank.move = () => {
      ...
      if(inputStates.up) {
        tank.moveWithCollisions(tank.frontVector.multipleByFloats(-tank.speed, -tank.speed, -tank.speed));
      }
      if(inputStates.down) {
        tank.moveWithCollisions(tank.frontVector.multipleByFloats(tank.speed, tank.speed, tank.speed));
      }
